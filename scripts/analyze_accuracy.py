...
def analyze():
    ...
    # ---- /correctscore মার্কেট: predicted probability bucket vs আসল hit-rate ----
    # best_pick-এর মতোই same-shape bucket-accuracy, কিন্তু correctscore_pick
    # ফিল্ড ব্যবহার করে (top_correct_scores()-এর সবচেয়ে সম্ভাব্য স্কোর) —
    # check_results.py সেভ করে correctscore_pick_status ("correct"/"incorrect"/
    # "no_pick"/"unresolved")। এই মার্কেটের own bucket-accuracy আলাদা করে
    # দেখা দরকার কারণ exact scoreline predict করা 1X2/BTTS/O-U-এর চেয়ে
    # স্বভাবতই অনেক বেশি কঠিন — একই probability-bucket-এ এদের hit-rate
    # ভিন্ন হওয়াই স্বাভাবিক, একসাথে মিশিয়ে দেখলে বিভ্রান্তিকর হতো।
    print("=== /correctscore মার্কেট: predicted probability vs আসল hit-rate ===")
    for lo, hi in PROBABILITY_BUCKETS:
        bucket_entries = []
        for e in entries:
            pick = e.get("correctscore_pick")
            if not pick or pick.get("probability_pct") is None:
                continue
            if lo <= pick["probability_pct"] < hi:
                bucket_entries.append(e)
        if not bucket_entries:
            continue
        b_correct = sum(1 for e in bucket_entries if e.get("correctscore_pick_status") == "correct")
        b_decided = sum(1 for e in bucket_entries if e.get("correctscore_pick_status") in ("correct", "incorrect"))
        if b_decided:
            actual_rate = round(100 * b_correct / b_decided, 1)
            print(f"  predicted {lo}-{hi - 1}%: n={b_decided}, আসল hit-rate={actual_rate}%")
    print("(correctscore_pick_status পুরনো এন্ট্রিতে নেই — এই ফিচারের পর থেকে জমা হওয়া ম্যাচেই দেখা যাবে)")
