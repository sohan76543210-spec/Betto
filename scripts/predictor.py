...
def predict_match(home_team_id, away_team_id, max_goals=10, deep=None, match_id=None, match_date=None):
    ...
    market_probs = _normalize_score_probs(
        dixon_coles_adjustment(poisson_probs, home_xg, away_xg, rho=DIXON_COLES_RHO)
    )
    ...
    for (hg, ag), p in market_probs.items():
        if hg > ag: hw += p
        elif hg == ag: dr += p
        else: aw += p
    ...
    return {
        ...
        "_raw_probs": raw,
        "_market_score_probs": market_probs,
    }
...
def _apply_empirical_score_calibration(probs, calibration):
    ...
    if temperature <= 0:
        temperature = 1.0
    temperature = max(0.50, min(2.00, temperature))
...
def correct_score_pick(top_scores, min_probability_pct=15.0, min_edge_pct=1.0):
