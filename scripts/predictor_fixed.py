...
- leakage-safe H2H / recent-form / venue-form signals
- Poisson + Dixon-Coles
- optional empirically learned exact-score calibration from model_config
...
# আগে এই তিনটা এখানেই হার্ডকোড ছিল — এখন data/model_config.json থেকে লোড হয়
# (ফাইল না থাকলে/অবৈধ হলে নিচের একই ডিফল্টে ফলব্যাক হয়, model_config.py-এর
# DEFAULT_CONFIG দেখুন)। build_calibration_set.py সেরা কম্বিনেশন পেলে এই
# ফাইলে লিখে দেয়, ফলে predictor.py এডিট/redeploy ছাড়াই নতুন weight কাজ করে।
_cfg = load_config()
H2H_WEIGHT = _cfg["h2h_weight"]
FORM_WEIGHT = _cfg["form_weight"]
VENUE_WEIGHT = _cfg["venue_weight"]
RECENCY_HALF_LIFE = _cfg["half_life"]
DIXON_COLES_RHO = _cfg["rho"]
...
def _weighted_avg_or(pairs, default):
    """_weighted_avg()-এর ফলাফল None (অর্থাৎ কোনো ডেটাই নেই) হলে default
    ব্যবহার করে। আগে `_weighted_avg(...) or 1.2` লেখা হতো — কিন্তু Python-এ
    `0 or 1.2` == 1.2, তাই কোনো টিম সত্যিই ০ গোল গড়ে স্কোর/কনসিড করলে (একটা
    বৈধ, real ভ্যালু) সেটাকে ভুলভাবে "ডেটা নেই" ধরে জোর করে default (1.2)
    বসিয়ে দিতো। এখানে শুধু None-এর ক্ষেত্রেই default প্রযোজ্য হয়, 0.0 নিজের
    মতোই থেকে যায়।"""
    val = _weighted_avg(pairs)
...
def predict_match(home_team_id, away_team_id, max_goals=10, deep=None, match_id=None, match_date=None):
    ...
    h2h_h = recency_weighted_scored_conceded(h2h, home_team_id, half_life=RECENCY_HALF_LIFE)
    h2h_a = recency_weighted_scored_conceded(h2h, away_team_id, half_life=RECENCY_HALF_LIFE)
    # form_h/form_a এখন home_recent/away_recent-এর 'off-venue' অংশ (home_away/
    # away_home) থেকে নেওয়া হচ্ছে, পুরো recent লিস্ট থেকে না — আগে venue_h/venue_a
    # (home_home/away_away) ছিল form_h/form_a-এর হুবহু সাবসেট, ফলে ৪০%(form)+৪৫%
    # (venue) ওজন কার্যত একই ম্যাচগুলোর একাংশকে দুইবার গুনতো। এখন form ও venue
    # দুইটা ডিসজয়েন্ট ম্যাচ-সেট থেকে আসে: form = ভেন্যু-নিরপেক্ষ সাম্প্রতিক সামর্থ্য
    # (নিজের ভেন্যুর বাইরে খেলা ম্যাচ), venue = নির্দিষ্ট ভেন্যুতে পারফরম্যান্স।
    form_h = recency_weighted_scored_conceded(home_away, home_team_id, half_life=RECENCY_HALF_LIFE)
    form_a = recency_weighted_scored_conceded(away_home, away_team_id, half_life=RECENCY_HALF_LIFE)
    venue_h = recency_weighted_scored_conceded(home_home, home_team_id, half_life=RECENCY_HALF_LIFE)
    venue_a = recency_weighted_scored_conceded(away_away, away_team_id, half_life=RECENCY_HALF_LIFE)

    # নোট: আগে এখানে `_weighted_avg(...) or 1.2` লেখা ছিল — Python-এ `0 or 1.2`
    # == 1.2 হওয়ায়, কোনো টিমের real (বৈধ) গড় ঠিক ০ হলেও সেটাকে ভুলভাবে
    # "ডেটা নেই" ধরে নিতো। _weighted_avg_or() শুধু None-এর (সত্যিকার ডেটা-অভাব)
    # ক্ষেত্রেই ডিফল্ট বসায়, বৈধ ০ ভ্যালুকে ০-ই রাখে।
    hs = _weighted_avg_or([
    ...
    # IMPORTANT: Do not use current team-statistics/standings/injury/Elo values
    # in the scoring model here. Those sources are either cumulative/current-state
    # data (historical leakage risk) or stateful data that cannot be reproduced
    # consistently by the calibration pipeline. Production/backtest parity is
    # therefore kept to H2H + form + venue + home advantage + rest + Dixon-Coles.
    # The `deep` argument is retained for API compatibility.
    deep = deep or {}
    hstats = deep.get("home_team_stats")
    astats = deep.get("away_team_stats")

    # Rest-days / fixture congestion: uses only already-fetched recent matches;
    ...
    for hg in range(max_goals + 1):
        for ag in range(max_goals + 1):
            poisson_probs[(hg, ag)] = _poisson_prob(hg, home_xg) * _poisson_prob(ag, away_xg)

    # One probability model, one source of truth.
    # Dixon-Coles is the production exact-score distribution. Any empirical
    # correction must be learned from historical held-out data and supplied via
    # model_config; no hand-picked high/low-score boost is applied here.
    # Build ONE final score probability matrix. All downstream markets,
    # including 1X2 and Correct Score, are derived from this same matrix.
    market_probs = _normalize_score_probs(
        dixon_coles_adjustment(
            poisson_probs, home_xg, away_xg, rho=DIXON_COLES_RHO
        )
    )
    score_probs = _apply_empirical_score_calibration(
        market_probs, _cfg.get("correct_score_calibration")
    )
    final_score_probs = _normalize_score_probs(score_probs)

    hw = dr = aw = btts = over = 0.0
    for (hg, ag), p in final_score_probs.items():
        if hg > ag: hw += p
        elif hg == ag: dr += p
        else: aw += p
    ...
    most_score = max(score_probs, key=score_probs.get)
    ...
    return {
        ...
        "_score_probs": final_score_probs,
        "_raw_probs": raw,
        "_market_score_probs": final_score_probs,
    }
...
def high_odds_pick(prediction, min_probability=0.40, min_reliability=70):
...
def _normalize_score_probs(probs):
    """Return a clean probability distribution for scorelines.

    Dixon-Coles implementations can differ in whether they normalize after
    applying their correction.  Normalizing here makes the downstream market
    calculations and score calibration deterministic, without any API calls.
    """
    if not probs:
        return probs
    clean = {score: max(0.0, float(prob)) for score, prob in probs.items()}
    total = sum(clean.values())
    if total <= 0:
        return probs
    return {score: prob / total for score, prob in clean.items()}


def _apply_empirical_score_calibration(probs, calibration):
    """Apply only externally learned exact-score calibration.

    Expected config shape (optional)::
        {
          "temperature": 1.0,
          "score_multipliers": {"1-1": 0.98, "2-1": 1.02}
        }

    Values are meant to be produced by a leakage-safe held-out backtest. If the
    section is absent/invalid, the Dixon-Coles distribution is returned unchanged.
    No API calls and no hard-coded score preferences are introduced here.
    """
    base = _normalize_score_probs(probs)
    if not base or not isinstance(calibration, dict):
        return base
    ...
    if temperature <= 0:
        temperature = 1.0
    temperature = max(1.00, min(2.00, temperature))

    multipliers = calibration.get("score_multipliers", {})
    if not isinstance(multipliers, dict):
        multipliers = {}

    adjusted = {}
    for score, prob in base.items():
        key = f"{score[0]}-{score[1]}"
        try:
            mult = float(multipliers.get(key, 1.0))
        except (TypeError, ValueError):
            mult = 1.0
        # Calibration files may contain noisy estimates; keep learned values
        # bounded so one scoreline cannot absorb the whole distribution.
        mult = max(0.75, min(1.25, mult))
        adjusted[score] = (max(0.0, prob) ** (1.0 / temperature)) * mult

    return _normalize_score_probs(adjusted)


def _calibrate_correct_score_probs(probs, home_xg, away_xg):
    """Backward-compatible wrapper with no unvalidated score reweighting."""
    return _normalize_score_probs(probs)

def top_correct_scores(prediction, relative_threshold=0.55, max_scores=3):
...
def correct_score_pick(top_scores, min_probability_pct=15.0, min_edge_pct=1.5):
    """top_correct_scores()-এর সবচেয়ে সম্ভাব্য স্কোরটাকেই আগে সবসময় "Correct
    Score" pick হিসেবে প্রকাশ করা হতো, probability যত কমই হোক না কেন — exact
    scoreline স্বভাবতই low-probability event (সাধারণত সেরা স্কোরও ~১০-২০%-এর
    বেশি হয় না), তাই সবসময় একটা পিক জোর করে প্রকাশ করা মানে বেশিরভাগ সময়ই
    ভুল প্রমাণিত হবে। এই ফাংশন দুইটা শর্ত পূরণ না হলে None রিটার্ন করে (অর্থাৎ
    ওই ম্যাচের জন্য কোনো Correct Score পিক প্রকাশিত হয় না):
      ১. top স্কোরের probability_pct অন্তত min_probability_pct হতে হবে
      ২. top ও দ্বিতীয় সম্ভাব্য স্কোরের মধ্যে অন্তত min_edge_pct ব্যবধান থাকতে
         হবে (কাছাকাছি হলে মডেলের 'best' বলে দাবি করা উচিত না, uncertainty
         থাকা উচিত)।"""
    if not top_scores:
        return None
    top = top_scores[0]
    if top["probability_pct"] < min_probability_pct:
        return None
    if len(top_scores) > 1:
        second = top_scores[1]
        if (top["probability_pct"] - second["probability_pct"]) < min_edge_pct:
            return None
    return top
...
def total_goals_distribution(prediction, n_buckets=7):
    """Return the compact ordinal total-goals distribution used by RPS tracking.

    Buckets are 0, 1, 2, ..., n_buckets-2, and n_buckets-1+.
    It is derived only from the already-computed score grid, so it performs no
    additional API request and does not change the prediction itself.
    """
    try:
    ...
    dist = [0.0] * n_buckets
    for (hg, ag), prob in prediction.get("_score_probs", {}).items():
        try:
            bucket = min(int(hg) + int(ag), n_buckets - 1)
            dist[bucket] += max(0.0, float(prob))
        except (TypeError, ValueError):
            continue
    ...
    return [round(x, 4) for x in dist]
