...
# football-data.org-এর নিজের কম্পিটিশন কোড — এই ১২টার মধ্যে যেগুলো এখন সিজন-অফ
# (World Cup ২০২৬ ইতিমধ্যে শেষ ১৯ জুলাই; Euro এই বছর হচ্ছেই না) — সেগুলো fdo-এর
# আউটপুট থেকে বাদ দেওয়া হয় যাতে এদের জন্য পরের ধাপে অযথা API কল না যায়।
# ⚠️ Champions League (CL) আগে এখানে বাদ দেওয়া ছিল এই ধরে নিয়ে যে মূল গ্রুপ/লিগ
# ফেজ সেপ্টেম্বরে শুরু হবে (তখনও শুরু হয়নি ধরে নেওয়া হয়েছিল) — কিন্তু লিগ ফেজ
# ইতিমধ্যে শুরু হয়ে গেছে, তাই CL বাদ দিলে ঐ ম্যাচগুলো prediction থেকে সম্পূর্ণ
# বাদ পড়ে যাচ্ছিল। তাই CL এখান থেকে সরানো হলো।
# ⚠️ কোডগুলো (WC, EC) football-data.org-এর স্ট্যান্ডার্ড কম্পিটিশন কোড ধরে
# নেওয়া হয়েছে — football_data_org.py দেখে নিশ্চিত করে নেওয়া ভালো এগুলোই ব্যবহার
# হচ্ছে কিনা।
EXCLUDED_FDO_CODES = {"WC", "EC"}
...
def get_matches_for_date(date_iso: str):
    """fdo-এর ১২ লিগ (অফ-সিজন ৩টা বাদে) + aps-এর বাকি লিগ (dedup করে) —
    এক তারিখের সব নির্ধারিত ম্যাচ।"""
    matches = []
    try:
        for m in fdo.get_matches_for_date(date_iso):
            code = (m.get("competition") or {}).get("code")
            if code in EXCLUDED_FDO_CODES:
                continue  # World Cup / Euro / Champions League — এখন সিজন-অফ বা শুধু কোয়ালিফায়ার
...
# football-data.org (fdo) নিজের status কোড ব্যবহার করে ("FINISHED", "SCHEDULED",
# "LIVE", "IN_PLAY", "PAUSED", "POSTPONED", "SUSPENDED", "CANCELLED", "AWARDED"),
# api-sports.io (aps) আলাদা কোড ব্যবহার করে ("FT", "NS", "1H", "HT", "2H", "PST",
# "CANC", "ABD", ...). check_results.py শুধু একটা কনভেনশন ("FT" = ফাইনাল ফলাফল
# পাওয়া গেছে ও নির্ভরযোগ্য) জানে — তাই এই রাউটারই দুই সোর্সের status normalize
# করে দেয়, যাতে ডাউনস্ট্রিম কোড সোর্স-নির্দিষ্ট স্ট্যাটাস কোড নিয়ে ভাবতে না হয়।
# এটা না করলে fdo-এর (প্রধান সোর্স, ১২টা বড় লিগ) কোনো ম্যাচই কখনো "final"
# হিসেবে ধরা পড়তো না, কারণ fdo কখনো "FT" ফেরত দেয় না।
_FINISHED_STATUSES = {"FT", "FINISHED", "AWARDED"}


def get_match_result(match_id):
    source, raw = _dec(match_id)
    if source == SOURCE_FDO:
        result = fdo.get_match_result(int(raw))
    elif source == SOURCE_APS:
        result = aps.get_match_result(int(raw))
    else:
        print(f"DEBUG: router get_match_result unknown source for {match_id!r}")
        return {"status": None, "home_goals": None, "away_goals": None}

    if result.get("status") in _FINISHED_STATUSES and result.get("home_goals") is not None:
        result = dict(result, status="FT")
    return result


def get_aps_results_by_date(date_iso: str):
    """check_results.py-এর জন্য: aps (api-sports.io)-সোর্সড pending entry-গুলো
    এক তারিখে একাধিক থাকলে, প্রতিটার জন্য আলাদা get_match_result() না করে
    একবারে পুরো তারিখের ফলাফল আনতে এটা ব্যবহার করুন — aps.get_results_by_date()
    এর মতোই, কিন্তু key composite id ("aps:123") হিসেবে re-encode করা।
    fdo-এর কোনো দৈনিক কোটা-ক্যাপ নেই বলে fdo entry-র জন্য এই ব্যাচিং করা হয়নি,
    সেগুলো এখনো get_match_result() দিয়ে আলাদাভাবেই চেক করা হয়।
    """
    raw_results = aps.get_results_by_date(date_iso)
    out = {}
    for raw_id, result in raw_results.items():
        composite_id = _enc(SOURCE_APS, raw_id)
        if result.get("status") in _FINISHED_STATUSES and result.get("home_goals") is not None:
            result = dict(result, status="FT")
        out[composite_id] = result
    return out
