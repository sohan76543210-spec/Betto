...
FALLBACK_APS_LEAGUES = {
    # --- ৫২টা থেকে বাছাই করা ৩০টা "এখন চলমান সিজন"-এর লীগ (১২টা কোর বাদে) ---
    # বাদ দেওয়া হয়েছে: World Cup (শেষ ১৯ জুলাই), Euro (২০২৬-এ হচ্ছে না),
    # Australia A-League (শুরু ১৬ অক্টোবর), UEFA Nations League (আগস্টে খেলা নেই),
    # UCL/Europa/Conference (শুধু কোয়ালিফায়ার, মূল স্টেজ সেপ্টেম্বরে),
    # + কিছু দ্বিতীয়-স্তরের/কম-গুরুত্বপূর্ণ লীগ (বাজেট বাঁচাতে)
    ("England","League One"),
    ("Belgium","First Division A"), ("Austria","Bundesliga"),
    ("Norway","Eliteserien"), ("Sweden","Allsvenskan"),
    ("Japan","J. League"), ("Saudi Arabia","Saudi Pro League"),
    ("Russia","Premier League"), ("Spain","Segunda División"),
    ...
    ("South Korea","K League 1"), ("China","Super League"),
    ("World","Copa Libertadores"),
    # --- নতুন যোগ করা লীগ (আগে কোডে ছিল না) ---
    # ⚠️ NOTE: এই লীগগুলোর league ID ভেরিফাই করা যায়নি (নেটওয়ার্ক অ্যাক্সেস নেই),
    # শুধু নাম দিয়ে যোগ করা হলো — DEBUG লগ চেক করে প্রথম রানের পর নাম মিলছে
    # কিনা যাচাই করে নিন।
    ("Colombia","Primera A"),
...
FALLBACK_APS_LEAGUE_IDS = {
    ...
    # নতুন যোগ করা লীগগুলোর (Colombia, Czech, Croatia, Serbia, Ukraine,
    # Paraguay, Ecuador, Romania) ID যাচাই করা যায়নি — নাম-ম্যাচিং দিয়েই
    # allow হবে। DEBUG লগ দেখে সঠিক ID পেলে এখানে যোগ করে দিন।
}
