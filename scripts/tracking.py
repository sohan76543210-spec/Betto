...
def _weighted_avg_or(pairs, default):
    """predictor._weighted_avg_or()-এর মতোই — শুধু None (ডেটা-অভাব) হলে
    default বসায়, বৈধ ০ ভ্যালুকে ভুল করে default দিয়ে replace করে না
    (আগের `... or 1.2` প্যাটার্নের বাগ এখানেও ছিল, একই কারণে ঠিক করা হলো)।"""
...
def _score_probs_for_match(m, h2h_w_n, form_w_n, venue_w_n, half_life, rho):
    ...
    from advanced_stats import recency_weighted_scored_conceded, dixon_coles_adjustment, dynamic_home_advantage
    ...
    # predictor.predict_match()-এর সাথে সামঞ্জস্যপূর্ণ থাকতে form/venue এখানেও
    # ডিসজয়েন্ট সেট থেকে নেওয়া হচ্ছে (home_away/away_home বনাম home_home/
    # away_away) — নইলে calibration একটা ভিন্ন (আগের, ওভারল্যাপিং) ফর্মুলার
    # জন্য weight অপ্টিমাইজ করে ফেলতো, যা লাইভ predictor.py-তে ব্যবহৃত
    # ফর্মুলার সাথে মিলতো না।
    form_h = recency_weighted_scored_conceded(m.get("home_away"), m["home_id"], half_life=half_life)
    form_a = recency_weighted_scored_conceded(m.get("away_home"), m["away_id"], half_life=half_life)
    ...
    hs = _weighted_avg_or([(h2h_h[0] if h2h_h else None, h2h_w_n),
                         (form_h[0] if form_h else None, form_w_n),
                         (venue_h[0] if venue_h else None, venue_w_n)], 1.2)
    hc = _weighted_avg_or([(h2h_h[1] if h2h_h else None, h2h_w_n),
                         (form_h[1] if form_h else None, form_w_n),
                         (venue_h[1] if venue_h else None, venue_w_n)], 1.2)
    a_s = _weighted_avg_or([(h2h_a[0] if h2h_a else None, h2h_w_n),
                          (form_a[0] if form_a else None, form_w_n),
                          (venue_a[0] if venue_a else None, venue_w_n)], 1.2)
    a_c = _weighted_avg_or([(h2h_a[1] if h2h_a else None, h2h_w_n),
                          (form_a[1] if form_a else None, form_w_n),
                          (venue_a[1] if venue_a else None, venue_w_n)], 1.2)

    # Production predictor applies the same dynamic home-advantage transform.
    # Keeping it here makes calibration optimize the model that is actually used
    # in live predictions instead of a simplified surrogate.
    h_mult, h_con_mult = dynamic_home_advantage(m.get("home_home"), m.get("home_away"), m["home_id"], default_boost=1.08)
    home_xg = max(0.15, min(4.5, ((hs * h_mult) + a_c) / 2.0))
    away_xg = max(0.15, min(4.5, (a_s + (hc * h_con_mult)) / 2.0))
...
def _log_loss_for_params(matches, h2h_w_n, form_w_n, venue_w_n, half_life, rho):
    ...
    for m in matches:
        ...
        p_actual = max(1e-6, probs.get((min(actual_hg, 10), min(actual_ag, 10)), 1e-6))
...
def run_calibration(matches_provider, h2h_weights=(0.10, 0.15, 0.20),
    ...
    ⚠️ Overfitting-গার্ড: matches-কে chronological train/test-এ ভাগ করা হয়।
    matches_provider() থেকে match_date থাকলে পুরনো ম্যাচগুলো train এবং সবচেয়ে
    নতুন ম্যাচগুলো held-out test হিসেবে ব্যবহৃত হয়; team traversal order-এর ওপর
    আর নির্ভর করা হয় না। সেরা কম্বিনেশন
    ...
    **held-out test সেটের ওপর** — কারণ যে ডেটার ওপর tune করা হয়েছে সেই একই
    ...
    # Newest observations are the realistic out-of-sample test set.
    matches = sorted(matches, key=lambda m: m.get("match_date") or "")
