...
ফলাফল-জানা ম্যাচের একটা লিস্ট রিটার্ন করে, প্রতিটায় থাকা দরকার:
    {"home_id", "away_id", "h2h", "home_recent", "away_recent",
     "home_home", "home_away", "away_home", "away_away",
...
def _auto_team_ids(max_teams=MAX_AUTO_TEAMS):
    ...
    """
    counter = Counter()
    for path in (HISTORY_PATH, LOG_PATH, PREDICTIONS_PATH):
        data = _load_json(path)
        if isinstance(data, dict):
            data = data.get("matches") or []
        for entry in data:
...
def matches_provider():
    ...
    samples = []
    seen_match_ids = set()
    # Fetch each team's larger recent-match pool once. For every historical
    # target match we then cut the pool to matches strictly BEFORE that target.
    # This prevents future-form leakage while avoiding one API call per sample.
    team_form_cache = {}
    h2h_cache = {}

    def _date_before(matches, target_date, limit=MATCHES_PER_TEAM):
        if not target_date:
            return (matches or [])[:limit]
        target_day = str(target_date)[:10]
        prior = [m for m in (matches or []) if str(m.get("utcDate") or "")[:10] < target_day]
        prior.sort(key=lambda x: x.get("utcDate") or "", reverse=True)
        return prior[:limit]
    ...
    for team_id in TEAM_IDS:
        if _quota_exhausted():
            ...
            break
        try:
            team_matches = team_form_cache.get(team_id)
            if team_matches is None:
                team_matches = football_api.get_team_recent_form(team_id, limit=100)
                team_form_cache[team_id] = team_matches
        ...
        for m in team_matches:
            ...
            target_date = m.get("utcDate")
            try:
                h2h_key = (home_id, away_id, str(target_date)[:10])
                h2h = h2h_cache.get(h2h_key)
                if h2h is None:
                    h2h_all = football_api.get_head_to_head(home_id, away_id, match_id=match_id, limit=100)
                    h2h = _date_before(h2h_all, target_date, limit=8)
                    h2h_cache[h2h_key] = h2h
                home_recent = _date_before(team_form_cache.get(home_id, []), target_date, limit=MATCHES_PER_TEAM)
                away_recent = _date_before(team_form_cache.get(away_id, []), target_date, limit=MATCHES_PER_TEAM)
            except Exception as e:
                print(f"DEBUG: enrichment failed for match {match_id}: {e}", file=sys.stderr)
                continue

            home_home, home_away = _venue_split(home_recent, home_id)
            away_home, away_away = _venue_split(away_recent, away_id)
            ...
            samples.append({
                ...
                "home_home": home_home,
                # home_away/away_home: venue-split-এর complement অংশ, এখন
                # tracking._score_probs_for_match()-এর form_h/form_a সিগন্যালে
                # ব্যবহার হয় (predictor.py-এর disjoint form/venue ফিক্সের সাথে
                # সামঞ্জস্যপূর্ণ থাকতে) — আগে এগুলো ফেলে দেওয়া হতো।
                "home_away": home_away,
                "away_home": away_home,
                "away_away": away_away,
    ...
    # Team-by-team API traversal order is NOT chronological. Sort globally so
    # calibration can use older matches for training and newer matches for the
    # held-out test set.
    samples.sort(key=lambda x: x.get("match_date") or "")
...
if __name__ == "__main__":
    ...
    from tracking import run_calibration, correctscore_calibration_report
    from model_config import load_config, write_config

    current = load_config()
    # matches একবারই fetch করা হচ্ছে (API কোটা সীমিত) — run_calibration()-কে
    # cache করা লিস্ট রিটার্ন করা একটা lambda দেওয়া হচ্ছে, আবার নতুন করে
    # matches_provider() (যেটা ফের API কল করবে) ডাকা হচ্ছে না। এই একই cached
    # matches পরে correctscore_calibration_report()-এও পুনর্ব্যবহার হয়।
    cached_matches = matches_provider()
    result = run_calibration(lambda: cached_matches, baseline=current)
    print(result)

    # /correctscore মার্কেটের জন্য আলাদা diagnostic — বর্তমান (production)
    # কম্বিনেশন এবং (যদি ভালো পাওয়া যায়) নতুন candidate কম্বিনেশন, দুইটার
    # জন্যই RPS + top-score bucket-accuracy বের করা হচ্ছে, একই cached_matches
    # দিয়ে (কোনো নতুন API কল ছাড়া)।
    correctscore_current = correctscore_calibration_report(cached_matches, params=current)
    correctscore_candidate = None
    if "error" not in result:
        correctscore_candidate = correctscore_calibration_report(cached_matches, params={
            "h2h_weight": result["h2h_weight"], "form_weight": result["form_weight"],
            "venue_weight": result["venue_weight"], "half_life": result["half_life"],
            "rho": result["rho"],
        })
    print("correctscore (current config):", correctscore_current)
    print("correctscore (candidate config):", correctscore_candidate)
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({
            ...
            # correctscore_*: h2h/form/venue/half_life/rho এই মার্কেট-নির্দিষ্টভাবে
            # tune হয় না (result-এর overall exact-scoreline log-loss দিয়েই সব
            # market-এর জন্য একসাথে tune হয়), কিন্তু এই দুইটা diagnostic দেখায়
            # /correctscore-এর RPS ও probability-bucket accuracy বর্তমান লাইভ
            # কনফিগে কেমন, আর নতুন candidate প্রয়োগ হলে (সব মার্কেটের জন্য ভালো
            # হলেও) correctscore-এর জন্য ভালো/খারাপ হতো কিনা।
            "correctscore_current_config": correctscore_current,
            "correctscore_candidate_config": correctscore_candidate,
